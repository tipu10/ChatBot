...
class CoreBot {
    ...
    renderer, routes, locales, theme, plugins, appId, defaultTyping, defaultDelay, defaultRoutes, inspector, }) {
        ...
        this.plugins = (0, plugins_1.loadPlugins)(plugins);
        this.theme = theme || {};
        this.defaultTyping =
    ...
    getString(id, session) {
        // @ts-ignore
        return (0, i18n_1.getString)(this.locales, session.__locale, id);
    }
    ...
    async input({ input, session, lastRoutePath, dataProvider, }) {
        ...
        if (dataProvider) {
            ...
            // @ts-ignore
            const userEvent = await dataProvider.saveEvent(Object.assign(Object.assign({}, parsedUserEvent), { userId, eventId: (0, ulid_1.ulid)(), createdAt: new Date().toISOString(), from: models_1.MessageEventFrom.USER, ack: models_1.MessageEventAck.RECEIVED }));
        }
        if (this.plugins) {
            await (0, plugins_1.runPlugins)(this.plugins, 'pre', input, session, lastRoutePath, undefined, undefined, dataProvider);
        }
        if (this.routes instanceof Function) {
            this.router = new routing_1.Router([
                ...(await (0, routing_1.getComputedRoutes)(this.routes, {
                    input,
                    session,
                    lastRoutePath,
                })),
        ...
        if (this.plugins) {
            await (0, plugins_1.runPlugins)(this.plugins, 'post', input, session, lastRoutePath, response, messageEvents, dataProvider);
        }
        if (dataProvider) {
            // TODO: save bot responses to db and update user with new session and new params
            for (const messageEvent of messageEvents) {
                // @ts-ignore
                const botEvent = await dataProvider.saveEvent(Object.assign(Object.assign({}, messageEvent), { userId, eventId: (0, ulid_1.ulid)(), createdAt: new Date().toISOString(), from: models_1.MessageEventFrom.BOT, ack: models_1.MessageEventAck.SENT }));
            }
        }
